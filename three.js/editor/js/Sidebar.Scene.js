...
function SidebarScene( editor ) {
	...
	function buildOption( object, draggable, closed ) {
		...
		if ( closed == undefined && nodeStates.has( object ) ) {
	...
	function getMaterialName( object, material ) {
		...
		if ( material.name === '' && object.geometry.type === 'BasiliskBufferGeometry' )
			return object.geometry.materialName();
		
		return material.name;

	}

	function getGeometryName( object ) {

		if ( object.geometry.name === '' && object.userData.type && object.userData.parameters ) {

			var param = object.userData.parameters;
			
			for ( let i in param ) {
				var p = param[i];
				if ( p.type === 'pstring' && p.value !== '(null)' )
					return p.value;
			}
			
		}
		
		return object.geometry.name;
	...
	function buildHTML( object ) {
		...
		if ( object.isMesh ) {
			...
			html += ` <span class="type Geometry"></span> ${ escapeHTML( getGeometryName( object ) ) }`;
			html += ` <span class="type Material"></span> ${ escapeHTML( getMaterialName( object, material ) ) }`;
		...
		if ( object.geometry && object.geometry.error )
	...
	var backgroundColor = new UIColor().setValue( '#808080' ).setMarginLeft( '8px' ).onInput( onBackgroundChanged );
	...
	function refreshUI() {
		...
		options.push( buildOption( camera, false, true ) );
	...
	editor.client.signals.onload.add( refreshUI );

	editor.signals.geometryUpdated.add( function( object ) {
