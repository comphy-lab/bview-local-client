...
class BasiliskBufferGeometry extends BufferGeometry {
	...
	toJSON() {
		...
		/**
		 * A Basilisk geometry is fully described by its drawing command: the
		 * vertex data is regenerated by the server on connect, and
		 * BasiliskGeometryFromJSON() reconstructs from `parameters` alone.
		 *
		 * BufferGeometry.toJSON() would instead serialise every attribute.
		 * With autosave on, a scene with a few hundred thousand vertices turns
		 * into tens of megabytes of JSON on the main thread, on every one of
		 * the ten signals wired to saveState() -- which is felt as a long
		 * stall roughly a second after each change in the sidebar.
		 *
		 * Emitting only the parameters keeps saves small and constant-time,
		 * and loses nothing: the buffers were never ours to persist.
		 */
		...
		const data = {
			metadata: {
				version: 4.5,
				type: 'BufferGeometry',
				generator: 'BasiliskBufferGeometry.toJSON'
			},
			uuid: this.uuid,
			type: this.type,
		...
		if ( this.order !== undefined ) data.order = this.order;

		for ( let p in this.parameters ) {

			if ( p !== 'object' ) data[ p ] = this.parameters[ p ];
